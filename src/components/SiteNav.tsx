    ...
    >
      ...
      >
        ...
        <button
          ...
          aria-expanded={open}
          className="grid h-10 w-10 place-items-center rounded-xl bg-secondary text-foreground transition active:scale-90 md:hidden"
          onClick={() => setOpen((v) => !v)}
        >
      ...
      >
        ...
        <div
          onClick={() => setOpen(false)}
          className={`fixed inset-0 top-0 bg-ink/30 backdrop-blur-sm transition-opacity duration-200 ${
          ...
          <div className="flex flex-col gap-1">
            {links.map((l) => (
              <Link
                ...
                onClick={() => setOpen(false)}
                activeOptions={{ exact: l.to === "/" }}
                className="rounded-xl px-4 py-3 text-base font-semibold text-foreground/90 transition active:scale-[0.98] active:bg-cream data-[status=active]:bg-secondary data-[status=active]:text-foreground"
              >
                {l.label}
              </Link>
            ))}
            ...
            <a
              href={business.phoneHref}
              onClick={() => setOpen(false)}
              className="mt-1 inline-flex items-center justify-center gap-2 rounded-xl bg-ink px-4 py-3 text-base font-semibold text-ink-foreground transition active:scale-[0.98]"
            >
