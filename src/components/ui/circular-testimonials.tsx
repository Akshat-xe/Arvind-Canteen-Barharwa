        ...
        <div ref={containerRef} className="relative mx-auto flex h-80 w-full max-w-sm items-center justify-center sm:h-96">
          {testimonials.map((t, i) => (
            <button
              ...
              className="absolute h-64 w-48 overflow-hidden rounded-[1.75rem] shadow-lift focus-visible:outline-none sm:h-72 sm:w-52"
        ...
        <div>
          ...
          <div className="mt-7 flex gap-3">
            <button
              ...
              className="grid h-11 w-11 place-items-center rounded-full bg-ink text-ink-foreground transition hover:bg-primary active:scale-95"
            >
          ...
          <div className="mt-4 flex gap-1.5">
            {testimonials.map((_, i) => (
              <button
                key={i}
                onClick={() => { setActiveIndex(i); stop(); }}
                className={`h-1.5 rounded-full transition-all duration-300 ${i === activeIndex ? "w-6 bg-primary" : "w-1.5 bg-foreground/25"}`}
